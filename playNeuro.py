...
MODEL_NAME = "models/MOEX_model_10min_step_30_pred_5_50000_e50/model_3.0.2.keras"
FILE_NAME = 'BD/TATN_10_NOW.csv'
TIMESTEP = 30
PREDICTION = -5
VOLUME_DATA = 2000
...
new_df = pd.read_csv(FILE_NAME).tail(2000)

df0 = table.DataCreate(new_df)
df = df0.table
print(len(df))
df["target"] = df["MACD"].shift(PREDICTION)
...
ax1.plot(results_df.index, df['MACD'].values[TIMESTEP:], color='blue', linewidth=2)
ax1.plot(results_df.index, df['MACD_signal'].values[TIMESTEP:], color='red', linewidth=2)
ax1.set_title('Цены закрытия')
ax1.set_ylabel('Цена')
ax1.grid(True, alpha=0.3)

# ax2.plot(results_df.index, results_df['rsi_actual']*100, label='Фактический RSI', alpha=0.7)
ax2.plot(results_df.index, results_df['rsi_prediction'], label='Прогноз RSI', alpha=0.7)
ax2.set_title('MACD: Прогноз vs Факт')
ax2.set_ylabel('MACD chek')
ax2.legend()
...
ax4.plot(results_df.index, results_df['close'], label='RSI -20', alpha=0.7)
